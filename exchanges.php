...
class CoinGeckoImages {
    ...
    public function execute($testRun = false) {
        ...
        if (!file_exists("./images-exchanges")) {
            mkdir("./images-exchanges", 0777);
        }

        // NOTE: Nothing special, used for testing only
        // The testfile contains a couple coins for testing.
        if($testRun == true) {
            $json = json_decode(file_get_contents("./test/list-exchanges.json"), true);

        } else {
            $json = json_decode($this->httpRequest("https://api.coingecko.com/api/v3/exchanges/list"), true);
        }
        ...
        for ($i=0; $i <= $totalCount; $i++) { 
            ...
            if($this->fileCheck($json[$i]["id"]) == false) {
                ...
                $jsonTarget = json_decode($this->httpRequest("https://api.coingecko.com/api/v3/exchanges/" . $json[$i]["id"]), false);
                $imageUrl = $jsonTarget->image;
                ...
                if($imageUrl !== null) {
                    ...
                    $filename = strtolower("./images-exchanges/" . $json[$i]["id"] . "." . $extension);
    ...
    private function fileCheck($id) {
        ...
        foreach([".png", ".jpg", ".webp", ".jpeg", ".ico", ".svg", ".gif", "."] as $extension) {
            if(file_exists("./images-exchanges/" . $id . $extension)) {
                return true;
            }
        }
